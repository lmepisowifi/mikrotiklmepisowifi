...
:local changelogNote     "[lmepisowifi] updated.";
...
:do {
    ...
    :if ($optSz > 0) do={
        ...
        :if ([:len [:tostr $p2]] > 0) do={
            ...
            # Replace | with %0A for URL-safe messages (Telegram/Discord)
            # and with real \n for system note
            :local clOut "";
            :local clNoteOut "";
            :local clLen [:len $rv];
            ...
            :while ($ci < $clLen) do={
                ...
                :if ($ch = "|") do={
                    :set clOut ($clOut . "%0A");
                    :set clNoteOut ($clNoteOut . "\n");
                } else={
                    :set clOut ($clOut . $ch);
                    :set clNoteOut ($clNoteOut . $ch);
                }
                :set ci ($ci + 1);
            }
            :set changelog $clOut;
            :set changelogNote $clNoteOut;
        }
    }
    :do { /file remove [find name="hs-options.txt"] } on-error={}
} on-error={
...
:if ($localVersion != $ghVersion) do={
    ...
    # Set system login note with version and changelog
    :do {
        /system note set show-at-login=yes \
            note=("[lmepisowifi] v" . $ghVersion . "\n\nChangelog:\n" . $changelogNote);
        :log info "HotspotSync: System note updated with changelog.";
    } on-error={ :log warning "HotspotSync: Failed to set system note."; }
